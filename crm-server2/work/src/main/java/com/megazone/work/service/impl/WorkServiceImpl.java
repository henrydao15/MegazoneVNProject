...
//            throw new CrmException(WorkCodeEnum.WORK_CREATE_NAME_NULL_ERROR);
	...
	public List<WorkTaskTemplateClassVO> queryTaskByWorkId(WorkTaskTemplateBO workTaskTemplateBO) {
		...
		item.setClassId(-1).setClassName("(ungrouped)");
	...
	public Dict excelImport(MultipartFile file, Integer workId) throws IOException {
		...
		ExcelUtil.readBySax(file.getInputStream(), 0, (int sheetIndex, int rowIndex, List<Object> rowList) -> {
			if (rowIndex > 1001) {
				rowList.add(0, "Up to 1000 pieces of data can be imported at the same time");
				errList.add(rowList);
				return;
			}
			if (rowIndex > 1) {
				...
				if (StrUtil.isEmptyIfStr(rowList.get(0))) {
					rowList.add(0, "Task name cannot be empty");
					errList.add(rowList);
					return;
				}
				if (!StrUtil.isEmptyIfStr(rowList.get(2))) {
					...
					if (!BaseUtil.isTime(time)) {
						rowList.add(0, "The format of the start time and date is incorrect, for example: 2000-01-01");
						errList.add(rowList);
						return;
					}
				}
				if (!StrUtil.isEmptyIfStr(rowList.get(3))) {
					...
					if (!BaseUtil.isTime(time)) {
						rowList.add(0, "The format of the end time and date is wrong, for example: 2000-01-01");
						errList.add(rowList);
						return;
					}
				}
				if (StrUtil.isEmptyIfStr(rowList.get(6))) {
					rowList.add(0, "The task list cannot be empty");
					errList.add(rowList);
					return;
				}
				...
				if (StrUtil.isNotEmpty(mainUserName)) {
					if (ObjectUtil.isEmpty(workUserMap.get(mainUserName))) {
						rowList.add(0, "The person in charge does not match the current project member");
						errList.add(rowList);
						return;
					} else {
				...
				if (StrUtil.isNotEmpty(ownerUserName)) {
					...
					for (String userName : ownerUserNameList) {
						if (ObjectUtil.isEmpty(workUserMap.get(userName))) {
							rowList.add(0, "Participant does not match current project member");
							errList.add(rowList);
							return;
						}
					}
		...
		if (errList.size() > 0) {
			...
			try {
				...
				writer.writeHeadRow(Arrays.asList("Error Information", "Task Name(*)", "Task Description", "Start Time", "End Time", "Responsible Person", "Participant", "Task List (*)"));
	...
	/**
	 * Query exportable tasks
	 *
	 * @param workId project ID
	 * @return data
	 */
	@Override
	public List<Map<String, Object>> workTaskExport(Integer workId) {
		...
		recordList.forEach(record -> {
			...
			if (record.containsKey("mainUserId")) {
				Long mainUserId = Long.parseLong(record.get("mainUserId").toString());
				record.put("mainUserName", UserCacheUtil.getUserName(mainUserId));
			} else {
				record.put("mainUserName", "");
			}
			if (record.containsKey("createUserId")) {
				Long createUserId = Long.parseLong(record.get("createUserId").toString());
			...
			if (record.get("businessIds") != null) {
				...
				if (ids.size() > 0) {
					relateCrmWork.append("Business opportunity");
