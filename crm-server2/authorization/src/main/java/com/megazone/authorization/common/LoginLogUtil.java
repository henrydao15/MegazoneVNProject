...
public class LoginLogUtil {
	...
	public LoginLogEntity getLogEntity(HttpServletRequest request) {
		...
		if (ipCityInfo != null) {
			if (ipCityInfo.contains("Intranet IP")) {
				loginLogEntity.setLoginAddress("Intranet");
			} else {
				try {
					String[] addressArr = ipCityInfo.split("\\|");
					loginLogEntity.setLoginAddress(addressArr[0] + addressArr[2] + addressArr[3] + addressArr[4]);
				} catch (Exception e) {
					log.error("ip parsing exception:{}", e.getMessage());
				}
			}
		}
		UserAgent userAgent = UserAgentUtil.parse(request.getHeader("user-agent"));
		loginLogEntity.setDeviceType(userAgent.isMobile() ? "Mobile" : "Webpage");
