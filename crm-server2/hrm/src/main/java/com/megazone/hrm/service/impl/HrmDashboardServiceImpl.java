...
public class HrmDashboardServiceImpl implements IHrmDashboardService {
	...
	public Map<Integer, Long> employeeSurvey() {
		Result<JsonNode> auth = adminService.auth();
		JSONObject data = new JSONObject((ObjectNode) auth.getData());
	...
	public LastSalarySurveyVO lastSalarySurvey() {
		...
		if (CollUtil.isNotEmpty(sEmpRecordIds)) {
			...
			if (noDeptProportion.compareTo(new BigDecimal(0)) > 0) {
				...
				noDept.setDeptId(0);
				noDept.setDeptName("No Department");
	...
	public DoRemindVO toDoRemind() {
		...
		if (salaryMonthRecord != null && salaryMonthRecord.getExamineRecordId() != null) {
			Result<JsonNode> jsonObjectResult = crmExamineService.queryExamineRecordList(salaryMonthRecord.getExamineRecordId(), salaryMonthRecord.getCreateUserId());
			JSONObject jsonObject1 = new JSONObject((ObjectNode) jsonObjectResult.getData());
	...
	public List<NotesVO> queryNotesByTime(QueryNotesByTimeBO queryNotesByTimeBO) {
		...
		if (employeeObject != null && !employeeObject.isEmpty() && employeeObject.containsKey("index")) {
			...
			birthdayList.forEach(employee -> {
				String content = employee.getEmployeeName() + "birthday";
				if (employee.getAge() != null) {
					content = employee.getEmployeeName() + employee.getAge() + "birthday";
				}
			...
			entryList.forEach(employee -> {
				String content = employee.getEmployeeName() + "Job Day";
			...
			becomeList.forEach(employee -> {
				String content = employee.getEmployeeName() + "Regular day";
			...
			leaveList.forEach(employee -> {
				String content = employee.getEmployeeName() + "Resignation Date";
		...
		if (recruitObject != null && !recruitObject.isEmpty() && recruitObject.containsKey("manage")) {
			...
			recruitList.forEach(candidate -> {
				Date interviewTime = (Date) candidate.get("intreviewTime");
				boolean isAm = DateUtil.isAM(interviewTime);
				String format = DateUtil.format(interviewTime, "HH:mm");
				String content = candidate.get("candidateName") + (isAm ? "AM" : "PM") + format + "Interview";
	...
	/**
	 * Analysis of the proportion of employee status
	 */
	...
	/**
	 * Analysis of gender ratio of employees
	 */
	...
	/**
	 * Analysis of employee age ratio
	 */
	private List<TeamSurveyPropertiesVO> getAgeAnalysisList(BigDecimal allNum, List<HrmEmployee> employeeList) {
		...
		ageRangeMap.put(Range.lessThan(0), "Not filled");
		ageRangeMap.put(Range.closed(0, 17), "below 17");
		...
		ageRangeMap.put(Range.atLeast(56), "Above 56");
	...
	/**
	 * Analysis of the proportion of employees with age
	 */
	private List<TeamSurveyPropertiesVO> getCompanyAgeAnalysisList(BigDecimal allNum, List<HrmEmployee> employeeList) {
		...
		companyAgeRangeMap.put(Range.lessThan(0), "Not filled in");
		companyAgeRangeMap.put(Range.closed(0, 89), "within 3 months");
		companyAgeRangeMap.put(Range.closed(90, 179), "3-6 months");
		companyAgeRangeMap.put(Range.closed(180, 364), "6 months - 1 year");
		companyAgeRangeMap.put(Range.closed(365, 365 * 3 - 1), "1-3 years");
		companyAgeRangeMap.put(Range.closed(365 * 3, 365 * 5 - 1), "3-5 years");
		companyAgeRangeMap.put(Range.closed(365 * 5, 365 * 10 - 1), "5-10 years");
		companyAgeRangeMap.put(Range.atLeast(365 * 10), "10+ years");
	...
	public MySurveyVO mySurvey() {
		...
		if (salarySlip != null && salarySlip.getReadStatus() == 0) {
			String slipRemarks = UserCacheUtil.getUserName(salarySlip.getCreateUserId())
					+ "Updated your" + salarySlip.getMonth() + "month's salary slip, go to view>>";
	...
	public List<NotesVO> myNotesByTime(QueryNotesByTimeBO queryNotesByTimeBO) {
		...
		clockList.forEach(clock -> {
			String content = clock.getEmployeeName() + ClockType.valueOfName(clock.getClockType()) + "clock";
