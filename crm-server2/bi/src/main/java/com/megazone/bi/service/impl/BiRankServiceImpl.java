...
import com.megazone.bi.mapper.BiRankMapper;
...
public class BiRankServiceImpl implements BiRankService {
	...
	public JSONObject ranking(BiParams biParams) {
		//Contracts and repayment amounts are ranked and screened on the day they are selected,
		// and are calculated according to the current month, and the sum of the goals for these months or years is calculated
		// across months or years, and special treatment is required.
...
//        BiTimeUtil.dataFilter(menuId, record.getUserIds());
		...
		for (int i = 0; i < recordList.size(); i++) {
			...
			if (UserUtil.getUserId().equals(user.getLong("userId"))) {
				self = user.fluentPut("sort", i + 1).getInnerMapObject();
			}
		}
