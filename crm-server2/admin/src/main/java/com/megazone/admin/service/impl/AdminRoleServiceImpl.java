...
import com.megazone.admin.mapper.AdminRoleMapper;
...
public class AdminRoleServiceImpl extends BaseServiceImpl<AdminRoleMapper, AdminRole> implements IAdminRoleService {
	...
	public JSONObject auth(Long userId) {
		...
		if (redis.exists(cacheKey)) {
			return new JSONObject((ObjectNode) redis.get(cacheKey));
		}
		...
		if (jsonObject.containsKey("jxc")) {
			...
			if (jxc.containsKey("bi") && !jxc.getJSONObject("bi").isEmpty()) {
				...
				if (jsonObject.containsKey("bi")) {
					...
					bi.putAll(jxcBi.getInnerMapObject());
		...
		redis.setex(cacheKey, 300, jsonObject.node);
		return jsonObject;
	}
	...
	public void delete(Integer roleId) {
		...
		adminUserRoleService.removeByMap(object.getInnerMapObject());
		adminRoleMenuService.removeByMap(object.getInnerMapObject());
	}
	...
	public void setWorkRole(JSONObject object) {
		...
		if (roleId == null) {
			...
			updateById(adminRole);
			adminRoleMenuService.removeByMap(new JSONObject().fluentPut("role_id", roleId).getInnerMapObject());
		}
	...
	private String roleTypeCaseName(Integer type) {
		...
		switch (type) {
			case 1:
				name = "System Management Role";
				break;
			case 2:
				name = "Customer Management Role";
				break;
			case 7:
				name = "Office Management Role";
				break;
			case 8:
				name = "Project Management Role";
				break;
			case 9:
				name = "Human Resource Management Role";
				break;
			case 10:
				name = "Invoicing management role";
				break;
			default:
				name = "Custom Role";
		}
