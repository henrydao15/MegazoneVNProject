...
import com.megazone.oa.mapper.OaLogMapper;
...
public class OaLogServiceImpl extends BaseServiceImpl<OaLogMapper, OaLog> implements IOaLogService {
	...
	public void saveAndUpdate(JSONObject object) {
		...
		if (oaLogRelation != null) {
			logRelationService.removeByMap(new JSONObject().fluentPut("log_id", oaLog.getLogId()).getInnerMapObject());
	...
	private JSONObject getLogListObject(LogBO bo) {
		...
		UserInfo user = UserUtil.getUser();
		//getIntValue is empty will return 0
		Integer by = bo.getBy();
		if (by == null) {
			by = 0;
		}
		...
		if (bo.getType() == null) {
			//type== null is a custom time, you need to put the time in the parameter
	...
	private void saveLogRecord(Integer logId) {
		...
		OaLogRecord logRecord = BeanUtil.mapToBean(record.getInnerMapObject(), OaLogRecord.class, true);
	...
	public void deleteById(Integer logId) {
		...
		if (oaLog != null) {
			logRelationService.removeByMap(new JSONObject().fluentPut("log_id", logId).getInnerMapObject());
	...
	private JSONObject getCycleTime(Integer type) {
		...
		//Empty means the range is all employees
		...
		if (type.equals(1)) {
			...
			//dayOfWeek is 1 Sunday 2-7 Monday to Saturday and the database is 1-6 Monday to Saturday 7 Sunday so we need to convert the next
	...
	public List<Map<String, Object>> export(LogBO logBO) {
		...
		recordList.forEach((record -> {
			...
			// splicing comment reply content
			...
			list.add(record.fluentPut("sendName", sendName).fluentPut("relateCrmWork", relateCrmWork.trim()).fluentPut("comment", stringBuilder.toString().trim()).getInnerMapObject());
		}));
		return list;
	}
