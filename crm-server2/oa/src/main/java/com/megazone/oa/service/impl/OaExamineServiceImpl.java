...
import com.megazone.oa.common.FieldUtil;
...
import com.megazone.oa.entity.PO.*;
import com.megazone.oa.entity.VO.ExamineVO;
import com.megazone.oa.mapper.OaExamineLogMapper;
import com.megazone.oa.mapper.OaExamineMapper;
...
public class OaExamineServiceImpl extends BaseServiceImpl<OaExamineMapper, OaExamine> implements IOaExamineService {
	...
	public List<ExamineVO> transfer(List<ExamineVO> recordList) {
		...
		recordList.forEach(record -> {
			...
			//Super administrators, creators can update 2 review rejected 4 withdrawn approval
			...
			//Super administrators can delete 1 approval, 2 review rejection, 4 withdrawn approvals, and the creator can delete 4 withdrawn approvals
			...
			//The creator can withdraw the approval of 0 unreviewed 4 in review
			...
			if (record.getExamineStatus() == 2) {
				//If the approval is deleted, directly review 0
		...
		if (count > 0) {
			...
			switch (categoryTitle) {
				case "Travel approval":
					causeTitle.append("Itinerary, total");
	...
	public List<OaExamineField> getField(GetExamineFieldBO getExamineFieldBO) {
		...
		switch (oaType) {
			case 1:
				fieldUtil.oaFieldAdd("content", "Approval Content", "text", arr, 1, 0, oaExamineInfo.getContent(), "", 3, 1, "0,0").oaFieldAdd("remark", "Remark", "textarea", arr, 0, 0, oaExamineInfo.getRemark(), "", 3, 1, "1,0");
				break;
			case 2:
				fieldUtil.oaFieldAdd("type_id", "Leave type", "select", Lists.newArrayList("annual leave", "personal " +
						"leave", "sick leave", "maternity leave", "transfer leave", "marriage leave", "bereavement leave", "Other"), 1, 0, oaExamineInfo.getTypeId(), "", 3, 1, "0,0").oaFieldAdd("content", "Reason for leave", "text", arr, 1, 0, oaExamineInfo.getContent(), "", 3, 1, "1,0").oaFieldAdd("start_time", "starttime", "datetime", arr, 1, 0, oaExamineInfo.getStartTime(), "", 3, 1, "2,0").oaFieldAdd("end_time", "end time", "datetime", arr, 1, 0, oaExamineInfo.getEndTime(), "", 3, 1, "2,1").oaFieldAdd("duration", "Duration(days)", "floatnumber", arr, 1, 0, String.valueOf(oaExamineInfo.getDuration()), "", 3, 1, "3,0").oaFieldAdd("remark", "Remark", "textarea", arr, 0, 0, oaExamineInfo.getRemark(), "", 3, 1, "4,0");
				break;
			case 3:
				fieldUtil.oaFieldAdd("content", "Reasons for business trip", "text", arr, 1, 0, oaExamineInfo.getContent(), "", 3, 1, "0,0").oaFieldAdd("remark", "Remark", "textarea", arr, 0, 0, oaExamineInfo.getRemark(), "", 3, 1, "1,0").oaFieldAdd("cause", "Travel Details", "business_cause", arr, 1, 0, examineTravelList, "", 3, 1, "2,0").oaFieldAdd("duration", "Total travel days", "floatnumber", arr, 1, 0, String.valueOf(oaExamineInfo.getDuration()), "", 3, 1, "3,0");
				break;
			case 4:
				fieldUtil.oaFieldAdd("content", "Reason for overtime", "text", arr, 1, 0, oaExamineInfo.getContent(), "", 3, 1, "0,0").oaFieldAdd("start_time", "starttime", "datetime", arr, 1, 0, oaExamineInfo.getStartTime(), "", 3, 1, "1,0").oaFieldAdd("end_time", "end time", "datetime", arr, 1, 0, oaExamineInfo.getEndTime(), "", 3, 1, "1,1").oaFieldAdd("duration", "Total overtime days", "floatnumber", arr, 1, 0, oaExamineInfo.getDuration(), "", 3, 1, "2,0").oaFieldAdd("remark", "Remark", "textarea", arr, 0, 0, oaExamineInfo.getRemark(), "", 3, 1, "3,0");
				break;
			case 5:
				fieldUtil.oaFieldAdd("content", "Travel", "text", arr, 1, 0, oaExamineInfo.getContent(), "", 3, 1, "0,0").oaFieldAdd("cause", "Expense Details", "examine_cause", arr, 1, 0, examineTravelList, "", 3, 1, "1,0").oaFieldAdd("money", "Total Reimbursement Amount", "floatnumber", arr, 1, 0, String.valueOf(oaExamineInfo.getMoney()), "", 3, 1, "2,0").oaFieldAdd("remark", "Remark", "textarea", arr, 0, 0, oaExamineInfo.getRemark(), "", 3, 1, "3,0");
				break;
			case 6:
				fieldUtil.oaFieldAdd("content", "Reason for borrowing", "text", arr, 1, 0, oaExamineInfo.getContent(), "", 3, 1, "0,0").oaFieldAdd("money", "Loan amount (yuan)", "floatnumber", arr, 1, 0, String.valueOf(oaExamineInfo.getMoney()), "", 3, 1, "1,0").oaFieldAdd("remark", "Remark", "textarea", arr, 0, 0, oaExamineInfo.getRemark(), "", 3, 1, "2,0");
				break;
			default:
	...
	public void setOaExamine(JSONObject jsonObject) {
		...
		if (oaExamineTravelList != null) {
			for (int i = 0; i < oaExamineTravelList.size(); i++) {
				JsonNode json = oaExamineTravelList.node.get(i);
				OaExamineTravel oaExamineTravel = JSON.parseObject(json.asText(), OaExamineTravel.class);
		...
		if (oaExamineTravelList != null) {
			for (JsonNode json : oaExamineTravelList.node) {
				OaExamineTravel oaExamineTravel = JSON.parseObject(json.asText(), OaExamineTravel.class);
	...
	/**
	 * Supplementary approval field information
	 *
	 * @param label
	 * @param typeId
	 * @param recordId
	...
	/**
	 * @param examineType 1 pending review 2 passed 3 rejected
	 */
	...
	private void saveField(JSONArray array, String batchId) {
		...
		List<OaExamineField> oaExamineFields = JSON.parseArray(array.toJSONString(), OaExamineField.class);
	...
	public void oaExamine(AuditExamineBO auditExamineBO) {
		...
		//Query the audit record according to the audit record id
		...
		if (status == 4) {
			...
			OaExamineLog oaExamineLog = examineLogService.lambdaQuery().eq(OaExamineLog::getRecordId, recordId).eq(OaExamineLog::getExamineUser, auditUserId).eq(examineRecord.getExamineStepId() != null, OaExamineLog::getExamineStepId, examineRecord.getExamineStepId()).eq(OaExamineLog::getExamineStatus, 0).ne(OaExamineLog::getIsRecheck, 1).orderByDesc(OaExamineLog::getCreateTime).last("limit 1").one();
			//[Determine whether the current approver has approval authority
		...
		//Query approval type
		OaExamineCategory examineCategory = examineCategoryService.getById(examine.getCategoryId());
		//Query the current approval step
		OaExamineStep examineStep = examineStepService.getById(examineRecord.getExamineStepId());
		;
		//Query the current audit log
		Long createUserId = examine.getCreateUserId();
		OaExamineLog log;
		if (examineCategory.getExamineType() == 1) {
			log = examineLogService.lambdaQuery().eq(OaExamineLog::getRecordId, examineRecord.getRecordId()).eq(OaExamineLog::getExamineStepId, examineRecord.getExamineStepId()).eq(OaExamineLog::getExamineUser, auditUserId).eq(OaExamineLog::getIsRecheck, 0).last("limit 1").one();
		} else {
			log = examineLogService.lambdaQuery().eq(OaExamineLog::getRecordId, examineRecord.getRecordId()).eq(OaExamineLog::getExamineUser, auditUserId).eq(OaExamineLog::getExamineStatus, 0).eq(OaExamineLog::getIsRecheck, 0).last("limit 1").one();
		}
		...
		if (status == 2) {
			...
			//First query the first step of the approval step of the approval process
			...
			if (examineCategory.getExamineType() == 1) {
				...
				Integer orderId;
				Optional<OaExamineLog> oaExamineLogOpt = examineLogService.lambdaQuery().eq(OaExamineLog::getRecordId, recordId).eq(OaExamineLog::getIsRecheck, 0).ne(OaExamineLog::getExamineStatus, 0).orderByDesc(OaExamineLog::getOrderId).last("limit 1").oneOpt();
			...
			//examination passed
			//Determine the approval process type
			OaExamineStep nextExamineStep = null;
			boolean flag = true;
			if (examineCategory.getExamineType() == 1) {
				//fixed approval
				//Query the next approval step
				...
				//Determine whether it is a parallel sign
				if (examineStep.getStepType() == 3) {
					//Check whether the current check-in is completed
					//Current and signed personnel
					for (Long userId : TagUtil.toLongSet(examineStep.getCheckUserId())) {
						OaExamineLog examineLog = examineLogService.lambdaQuery().eq(OaExamineLog::getRecordId, examineRecord.getRecordId()).eq(OaExamineLog::getExamineStepId, examineRecord.getExamineStepId()).eq(OaExamineLog::getExamineUser, userId).ne(OaExamineLog::getIsRecheck, 1).last("limit 1").one();
						if (examineLog.getExamineStatus() == 0) {
							//and the sign is not finished
							flag = false;
							break;
						}
					}
					// and sign is not completed
					if (!flag) {
						examineRecord.setExamineStatus(3);
					}
				}
				if (flag) {
					// Determine if there is a next process
					if (nextExamineStep != null) {// have next process
						examineRecord.setExamineStatus(3);
	...
	public JSONObject queryExamineRecordList(String recordId) {
		...
		//If the current approval has been withdrawn
		...
		//Determine whether there is a withdrawal permission
		...
		if (oaExamineCategory.getExamineType() == 2) {
			...
			jsonObject.put("examineType", 1);
			//fixed approval
			List<Map<String, Object>> steps = examineStepService.listMaps(new LambdaQueryWrapper<OaExamineStep>().eq(OaExamineStep::getCategoryId, oaExamineCategory.getCategoryId()).orderByAsc(OaExamineStep::getStepNum));
			steps.forEach(step -> {
				if (ObjectUtil.equal(step.get("stepType"), 1)) {
					...
					//The audit log has been created
					if (logs != null && logs.size() > 0) {
						...
						//The audit log has not been created yet
						// query the person in charge
					...
					// first determine whether it has been reviewed
					List<JSONObject> logs = examineLogMapper.queryUserByRecordIdAndStepIdAndStatus(recordId, (long) step.get("stepId"));
					if (logs != null && logs.size() != 0) {
						//The audit log has been created
						int status = 0;
						...
						//This step has not been reviewed yet
						logs = new ArrayList<>();
					...
					if (logs != null && logs.size() != 0) {
						...
						//The audit log has not been created yet
						//Query the supervisor of the person in charge of the supervisor
	...
	/**
	 * Recursively query the first-level approval steps of the supervisor of the previous reviewer
	 */
	private Long queryStep(OaExamineStep step, Integer recordId, Long ownerUserId) {
		Optional<OaExamineLog> oaExamineLogOpt = examineLogService.lambdaQuery().eq(OaExamineLog::getRecordId, recordId).eq(OaExamineLog::getExamineStepId, step.getStepId()).and(e -> e.eq(OaExamineLog::getIsRecheck, 0).or().isNull(OaExamineLog::getIsRecheck)).eq(OaExamineLog::getExamineStatus, 1).orderByDesc(OaExamineLog::getLogId).last("limit 1").oneOpt();
		if (!oaExamineLogOpt.isPresent()) {
			//Get the previous review step
...
// and step_num = (SELECT step_num FROM 72crm_oa_examine_step where step_id = ?) - 1
			OaExamineStep lastStep = examineStepService.lambdaQuery().eq(OaExamineStep::getCategoryId, step.getCategoryId()).apply("and step_num = (SELECT step_num FROM 72crm_oa_examine_step where step_id = {0}) - 1", step.getStepId()).last("limit 1").one();
			if (lastStep == null) {
	...
	public List<JSONObject> queryExamineLogList(Integer recordId) {
		...
		logs.add(new JSONObject().fluentPut("examine_status", 6).fluentPut("examine_time", examineRecord.getCreateTime()).fluentPut("orderId", 0).fluentPut("realname", UserCacheUtil.getUserName(examineRecord.getCreateUser())).fluentPut("remarks", "").fluentPut("user_id", examineRecord.getCreateUser()));
	...
	public List<Map<String, Object>> export(ExamineExportBO examineExportBO, ExamineInfoVo examineInfoVo, List<OaExamineField> fieldList) {
		...
		//According to different approval types, query sql is different
		if (type == 5 || type == 3) {
		...
		switch (type) {
			case 1:
				recordList.forEach(record -> {
					record.put("category", "General approval");
			...
			case 3:
				for (int i = 0; i < recordList.size(); i++) {
					...
					//The same approval, the repeated fields from the second one do not need to be displayed again
					if (i > 0 && record.getInteger("examineId").equals(recordList.get(i - 1).getInteger("examineId"))) {
						record.fluentPut("category", "").fluentPut("create_time", "").fluentPut("create_user_name", "").fluentPut("examine_status", "").fluentPut("examine_user_name", "").fluentPut("previous_examine_user_name", "").fluentPut("content", "").fluentPut("remark", "").fluentPut("duration", "").fluentPut("relateCrmWork", "");
					} else {
						record.put("category", "Travel approval");
						putRelateCrmWork(record);
					}
					list.add(record.getInnerMapObject());
				}
			...
			case 5:
				for (int i = 0; i < recordList.size(); i++) {
					...
					//The same approval, the repeated fields from the second one do not need to be displayed again
					if (i > 0 && record.getInteger("examineId").equals(recordList.get(i - 1).getInteger("examineId"))) {
						record.fluentPut("category", "").fluentPut("createTime", "").fluentPut("createUserName", "").fluentPut("examineStatus", "").fluentPut("examineUserName", "").fluentPut("previousExamineUserName", "").fluentPut("content", "").fluentPut("money", "").fluentPut("remark", "").fluentPut("relateCrmWork", "");
					} else {
