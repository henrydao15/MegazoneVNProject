...
import com.megazone.crm.mapper.CrmBackLogMapper;
...
public class CrmBackLogServiceImpl implements ICrmBackLogService {
	...
	public JSONObject num() {
		...
		JSONObject kv = new JSONObject((ObjectNode) redis.get(CrmCacheKey.CRM_BACKLOG_NUM_CACHE_KEY + userId.toString()));
		if (kv != null) {
			return kv;
		} else {
			kv = new JSONObject();
		...
		if (authList.contains("crm:contract:index")) {
			...
			if (CollUtil.isNotEmpty(ids)) {
				...
				wrapper.eq(CrmBackLogDeal::getCreateUserId, userId);
				wrapper.select(CrmBackLogDeal::getTypeId);
				List<Integer> dealIdList = backLogDealService.listObjs(wrapper, obj -> Integer.parseInt(obj.toString()));
		...
		redis.setex(CrmCacheKey.CRM_BACKLOG_NUM_CACHE_KEY + userId.toString(), 600, kv.node);
		return kv;
	}
