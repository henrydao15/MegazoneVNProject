...
import com.megazone.core.feign.admin.entity.DataTypeEnum;
import com.megazone.core.feign.admin.entity.SimpleUser;
import com.megazone.core.feign.admin.service.AdminService;
import com.megazone.core.feign.crm.entity.BiAuthority;
...
import com.megazone.crm.constant.MonthEnum;
...
import com.megazone.crm.mapper.CrmInstrumentMapper;
...
public class CrmInstrumentServiceImpl implements CrmInstrumentService {
	...
	/**
	 * Sales presentation to decimals - rounded up
	 */
	...
	/**
	 * Details of the sales presentation
	 *
	 * @param biParams parameter
	 * @return data
	 */
	@Override
		...
		for (int i = 1; i <= cycleNum; i++) {
			...
			if (StrUtil.isNotEmpty(biParams.getType())) {
				if ("yyyyMMdd".equals(record.getDateFormat())) {
					...
					objectMap.put("type", DateUtil.parse(beginTime.toString(), record.getDateFormat()).monthStartFromOne() + "month");
				}
			} else {
	...
	public JSONObject queryPerformance(BiParams biParams) {
		...
		kv.put("queryItem", b);
		JSONObject dataRecord = crmInstrumentMapper.queryPerformance(kv.getInnerMapObject());
	...
	public BasePage<CrmActivity> queryRecordList(BiParams biParams) {
		...
		BasePage<CrmActivity> page = crmActivityMapper.queryRecordList(biParams.parse(), Dict.create().set("crmType", crmType).set("type", type).set("userIds", userIds).set("startTime", startTime).set("endTime", endTime).set("queryType", queryType).set("search", search));
	...
	public List<JSONObject> queryRecordCount(BiParams biParams) {
		...
		//1. Today 2. Yesterday 3. This week 4. Last week 5. This month 6. Last month 7. This quarter 8. Last quarter 9. This year 10 last year
		Integer type = BiTimeUtil.analyzeType(biParams.getType());
	...
	public JSONObject importRecordList(MultipartFile file, Integer crmType) {
		...
		ExcelUtil.readBySax(filePath, 0, (int sheetIndex, int rowIndex, List<Object> rowList) -> {
			...
			if (rowIndex > 1) {
				...
				if (StrUtil.isEmptyIfStr(rowList.get(0))) {
					rowList.add(0, "Follow-up content cannot be empty");
					errList.add(rowList);
					return;
				}
				if (StrUtil.isEmptyIfStr(rowList.get(1))) {
					rowList.add(0, "Creator cannot be empty");
					errList.add(rowList);
					return;
				}
				if (StrUtil.isEmptyIfStr(rowList.get(2))) {
					rowList.add(0, "belongs to" + crmEnum.getRemarks() + "cannot be empty");
					errList.add(rowList);
					return;
				}
				...
				if (createUserId == null) {
					rowList.add(0, "Creator does not exist");
					errList.add(rowList);
					return;
				}
				...
				switch (crmEnum) {
					case CUSTOMER:
						...
						CrmCustomer crmCustomer = crmCustomerService.lambdaQuery().select(CrmCustomer::getCustomerId).eq(CrmCustomer::getCustomerName, crmTypeName).ne(CrmCustomer::getStatus, 3).orderByDesc(CrmCustomer::getCreateTime).last(" limit 1 ").one();
						if (crmCustomer == null) {
							rowList.add(0, "The client does not exist");
							errList.add(rowList);
							return;
						}
						...
						if (StrUtil.isNotEmpty(contactsNames)) {
							...
							for (String name : names) {
								Optional<CrmContacts> crmContacts = crmContactsService.lambdaQuery().select(CrmContacts::getContactsId).eq(CrmContacts::getName, name).last("limit 1").oneOpt();
						...
						if (StrUtil.isNotEmpty(businessNames)) {
							...
							names.removeIf(StrUtil::isEmpty);
							List<CrmBusiness> list = crmBusinessService.lambdaQuery().select(CrmBusiness::getBusinessId).in(CrmBusiness::getBusinessName, names).ne(CrmBusiness::getStatus, 3).list();
						...
						break;
					case LEADS:
						CrmLeads crmLeads = crmLeadsService.lambdaQuery().select(CrmLeads::getLeadsId).eq(CrmLeads::getLeadsName, crmTypeName).orderByDesc(CrmLeads::getCreateTime).last(" limit 1 ").one();
						if (crmLeads == null) {
							rowList.add(0, "The clue does not exist");
							errList.add(rowList);
							return;
						}
						...
						break;
					case CONTACTS:
						CrmContacts crmContacts = crmContactsService.lambdaQuery().select(CrmContacts::getContactsId).eq(CrmContacts::getName, crmTypeName).orderByDesc(CrmContacts::getCreateTime).last(" limit 1 ").one();
						if (crmContacts == null) {
							rowList.add(0, "The contact does not exist");
							errList.add(rowList);
							return;
						}
						...
						break;
					case BUSINESS:
						CrmBusiness crmBusiness = crmBusinessService.lambdaQuery().select(CrmBusiness::getBusinessId).eq(CrmBusiness::getBusinessName, crmTypeName).ne(CrmBusiness::getStatus, 3).orderByDesc(CrmBusiness::getCreateTime).last(" limit 1 ").one();
						if (crmBusiness == null) {
							rowList.add(0, "The business opportunity does not exist");
							errList.add(rowList);
							return;
						}
						...
						break;
					case CONTRACT:
						CrmContract crmContract = crmContractService.lambdaQuery().select(CrmContract::getContractId).eq(CrmContract::getNum, crmTypeName).ne(CrmContract::getCheckStatus, 7).orderByDesc(CrmContract::getCreateTime).last(" limit 1 ").one();
						if (crmContract == null) {
							rowList.add(0, "The owning contract does not exist");
							errList.add(rowList);
							return;
						}
		...
		if (errList.size() > 0) {
			...
			try {
				...
				writer.merge(columnNum, "System user import template (*) is required");
				...
				if (crmType == 2) {
					writer.writeHeadRow(Arrays.asList("Error information", "Follow-up content(*)", "Creator(*)", "Customer(*)", "Follow-up time", "Follow-up method", "Related Contacts", "Related Opportunities"));
				} else {
					writer.writeHeadRow(Arrays.asList("Error information", "Follow-up content(*)", "Creator(*)", "Belonging" + crmEnum.getRemarks() + "(*)", "Follow-up time ", "Follow-up method"));
				}
