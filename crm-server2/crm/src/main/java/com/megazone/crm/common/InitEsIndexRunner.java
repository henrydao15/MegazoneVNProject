...
import com.megazone.crm.entity.PO.CrmCustomerPoolRelation;
...
public class InitEsIndexRunner implements ApplicationRunner {
	...
	private void initData(CrmEnum crmEnum) {
		...
		while (true) {
			...
			lastId = Integer.valueOf(o.toString());
			dataMap.put("lastId", lastId);
			log.warn("id:{},id{}", lastId, Thread.currentThread().getName());
			futureList.add(ActionRecordUtil.THREAD_POOL.submit(new SaveES(crmEnum, typeMap, mapList)));
		}
        /*
          ,
         */
		...
		while (true) {
			...
			log.warn("id:{},id{}", lastId, Thread.currentThread().getName());
			ActionRecordUtil.THREAD_POOL.execute(new SaveEsData(crmEnum, dataBOS));
		}
	...
	public class SaveES implements Callable<Boolean> {
		...
		@Override
		public Boolean call() {
			log.warn("id{}", Thread.currentThread().getName());
			BulkRequest bulkRequest = new BulkRequest();
